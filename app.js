(() => {
  ...
  function renderHome() {
    ...
    const personalizedCount = activeEntries.filter(entry => (entry.babyTips || []).length).length;
    const recent = activeEntries.slice(0, 4);

    root.innerHTML = `
      <div class="hero">
        <div class="hero-content">
          <span class="eyebrow">个人知识体系 · 本地优先</span>
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card"><div class="stat-number">${personalizedCount}</div><div class="stat-label">通用育儿建议</div></div>
  ...
  function renderSettings() {
    ...
    root.innerHTML = `
      <div class="panel">
        ...
        <div class="form-grid">
          ...
          <div class="field full">
            <label style="display:flex;align-items:center;gap:8px"><input id="personalize" type="checkbox" ${settings.personalize ? "checked" : ""}> 回答中优先使用个性化阶段建议</label>
            <small>开启后，可在未来导入包含宝宝阶段建议的知识包。</small>
          </div>
        </div>
      ...
      <div class="panel" style="margin-top:16px">
        <h3>应用说明</h3>
        <p class="muted">当前问答是本地检索式问答：它只使用已收录知识，不依赖外部人工智能服务，因此可离线运行，也不会把问题上传到服务器。它适合调用和整合你的个人知识体系，但不替代医生、心理师或其他专业人员的评估。</p>
      </div>
    `;
    root.querySelector("#saveSettings").addEventListener("click", () => {
      ...
      settings.personalize = root.querySelector("#personalize").checked;
      saveJson(SETTINGS_KEY, settings);
      toast("设置已保存");
    });
  }
  ...
  function openArticle(id) {
    ...
    els.articleDialogContent.innerHTML = `
      <article class="article-body">
        ...
        ${settings.personalize && (entry.babyTips || []).length ? `<h3>宝宝阶段建议</h3><div class="baby-tips-box"><ul>${entry.babyTips.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div>` : ""}
  ...
  function generateAnswerHtml(question) {
    ...
    const personalized = settings.personalize ? entries.flatMap(entry => entry.babyTips || []).map(item => item) : [];
