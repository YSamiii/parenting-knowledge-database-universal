(() => {
  ...
  function renderChecklists(initialAge = "") {
    ...
    const all = (Array.isArray(window.LOOKSEE_CHECKLISTS) && window.LOOKSEE_CHECKLISTS.length
      ? window.LOOKSEE_CHECKLISTS
      : window.BUILTIN_KNOWLEDGE_PACKAGE.checklists) || [];
    if (!all.length) {
      root.innerHTML = renderEmpty("清单内容未能载入", "请刷新页面；如果仍然只看到标题，请安装最新修正版。", "!");
      return;
    }
